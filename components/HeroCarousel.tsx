...
export default function HeroCarousel({ items, eyebrow, heading, body, ctaLabel = "Request a Quote", ctaUrl = "/contact", catalogLabel = "Download Catalog", catalogUrl = "/contact" }: { items: GalleryItem[]; eyebrow?: string; heading?: string; body?: string; ctaLabel?: string; ctaUrl?: string; catalogLabel?: string; catalogUrl?: string }) {
  ...
  return <section className={`home-hero ${slides.length ? "has-media" : ""}`}>
    ...
    <div className="hero-shade"/><div className="container hero-copy"><div className="eyebrow">{eyebrow || "Smart Lighting Solutions Since 2011"}</div><h1 className="display-title"><span className="hero-title-line">{plain}</span>{" "}<span>{accent}</span></h1><p className="lead">{body}</p><div className="hero-actions"><Link className="btn-primary" href={ctaUrl}>{ctaLabel}</Link><a className="btn-secondary" href={catalogUrl || "/contact"} target={catalogUrl?.startsWith("http") ? "_blank" : undefined}>{catalogLabel}</a></div></div>{slides.length > 1 && <div className="hero-dots">{slides.map((slide, itemIndex) => <button key={slide.id} className={itemIndex === index ? "active" : ""} onClick={() => setIndex(itemIndex)} aria-label={`Show banner ${itemIndex + 1}`}/>)}</div>}
  </section>;
}
