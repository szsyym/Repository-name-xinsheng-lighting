...
-- V3 additions are appended so fresh installations receive the same structure.
...
create table if not exists public.content_entries(id uuid primary key default gen_random_uuid(),type text not null check(type in('solution','article','knowledge','faq','case')),slug text unique not null,title text not null,excerpt text default '',content text default '',category text default '',cover_url text default '',faqs jsonb not null default '[]',related_products jsonb not null default '[]',seo jsonb not null default '{}',ai_summary text default '',status text default 'draft' check(status in('draft','published')),published_at date default current_date,created_at timestamptz default now(),updated_at timestamptz default now());
...
drop policy if exists "public read content entries" on public.content_entries;create policy "public read content entries" on public.content_entries for select using(status='published' or auth.role()='authenticated');
drop policy if exists "admin content entries" on public.content_entries;create policy "admin content entries" on public.content_entries for all to authenticated using(true) with check(true);
...
drop policy if exists "admin settings" on public.site_settings;create policy "admin settings" on public.site_settings for all to authenticated using(true) with check(true);
create index if not exists products_model_idx on public.products(model);create index if not exists content_entries_type_idx on public.content_entries(type,status,published_at desc);
